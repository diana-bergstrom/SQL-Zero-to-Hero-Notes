...
--Note that the WHERE statement can also be filtered by: WHERE first_name = 'Nick' AND last_name = 'Wahlberg';
...
--Let the query do your math for you - you can calculate percentages, round, etc. all within a query. The below query calculates the percentage of rental rate to replacement cost for the films and rounds it to 4 decimal places:
...
--EXISTS
--EXISTS checks to see if any rows are returned in a subquery. NOT EXISTS can be used to return anything that is not found in a subquery.
